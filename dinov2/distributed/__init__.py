...
def enable(*, set_cuda_current_device: bool = True, overwrite: bool = False, allow_nccl_timeout: bool = False):
    ...
    dist.init_process_group(backend="nccl", rank=torch_env.local_rank, world_size=torch_env.local_world_size, timeout=datetime.timedelta(seconds=5400)) #only backend arg previously
    dist.barrier()
